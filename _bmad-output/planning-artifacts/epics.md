...
- ARCH-1: Project initialized as monorepo — ces-frontend (React/Vite/TS), ces-backend-python (FastAPI), ces-backend-go (Gin), shared/ directory. Docker Compose brings up PostgreSQL 16 + Qdrant at start.
- ARCH-2: DB schema uses golang-migrate (Go) and Alembic (Python); both produce identical final schema. Shared baseline in `shared/schema/baseline.sql`. Tables: users, ddrs, ddr_dates, occurrences, corrections, keywords, processing_queue, pipeline_runs — all UUID v4 PKs, snake_case, timestamptz created_at/updated_at.
...
- ARCH-9: Keyword store is `shared/keywords.json` — single source of truth read by both backends. `PUT /keywords` writes file + triggers in-memory reload without restart.
...
- ARCH-14: Deployment target: single AWS EC2 instance (t3.medium/large), Docker Compose in production, Nginx reverse proxy + SSL termination. Both backend containers present during dual-backend phase; one active behind Nginx proxy.
- ARCH-15: Dual-backend parity: Go is canonical. Python must match Go behavior. Shared test fixtures in `shared/test-fixtures/`. Both must pass before backend selection decision.
- ARCH-16: OpenAPI spec auto-generated — FastAPI `/docs` + Go swaggo/swag. Both specs kept in sync as parity validation.
...
**When** `uvicorn app.main:app --reload` is run in `ces-backend-python/`
...
**Given** the Go backend scaffold exists
**When** `go run main.go` is run in `ces-backend-go/`
**Then** Gin server starts and `GET /health` returns `{ "status": "ok" }` with HTTP 200
**And** `internal/config/config.go` contains `Config` struct — all config from env vars

**Given** `.env.example` files exist in repo root, `ces-frontend/`, `ces-backend-python/`, `ces-backend-go/`
...
**Given** Go backend migrations directory `ces-backend-go/migrations/` exists
**When** `golang-migrate` runs `001_initial_schema.up.sql`
...
**Given** Python backend Alembic is configured in `ces-backend-python/alembic/`
...
**And** `shared/schema/baseline.sql` documents the canonical schema
...
**Given** both Go and Python backends implement the login endpoint
...
As a platform developer,
I want DDR, pipeline, and cost tracking tables created via migrations on both backends,
So that all extraction pipeline data has a durable, structured home before any pipeline code runs.

**Acceptance Criteria:**

**Given** migration `002_ddr_schema` runs on both backends
...
**And** Go and Python migrations produce identical schema
...
**Given** the 109-page and 229-page sample PDFs in `shared/test-fixtures/`
**When** pre-splitter test suite runs
**Then** 100% of date boundaries are correctly detected for both samples
**And** same fixture test passes on both Go and Python implementations
...
**When** `pipeline/extract` calls Gemini 2.5 Flash-Lite with `responseSchema` from `shared/schema/ddr_schema.json`
...
**And** Go errors are wrapped: `fmt.Errorf("pipeline.extract: %w", err)` — no bare `return err`
...
**Given** shared test fixtures in `shared/test-fixtures/expected_timelogs.json`
**When** extraction test suite runs against sample DDR fixture
**Then** same test passes on both Go and Python backends (parity validation)
...
**When** `shared/schema/baseline.sql` is reviewed
...
**Given** migration `003_occurrences` runs on both backends
**When** schema is inspected
...
**Given** `shared/keywords.json` is created
...
**And** both backends load it at startup via `keywords/loader`
**And** `shared/schema/baseline.sql` is updated to include occurrences table
...
**Given** `shared/keywords.json` is loaded at backend startup
...
**Given** `shared/test-fixtures/expected_occurrences.json` contains known input→type mappings
**When** keyword classification test suite runs
**Then** ≥ 90% of expected type classifications match (FR7 success criterion)
**And** same test passes on both Go and Python backends
...
**Given** `shared/test-fixtures/expected_occurrences.json` contains known mMD values
...
As a platform developer,
I want the corrections table created via migrations on both backends,
So that every occurrence edit can be durably stored with full metadata in an append-only audit trail.

**Acceptance Criteria:**

**Given** migration `004_corrections` runs on both backends
**When** schema is inspected
...
**And** Go and Python migrations produce identical schema
**And** `shared/schema/baseline.sql` updated to include corrections table
...
**Given** both Go and Python backends implement `PATCH /occurrences/:id`
...
**And** same cap and same summary format applied on both Go and Python backends
...
**Given** both Go (excelize) and Python (openpyxl) backends implement the export
...
**Given** BM25 test suite runs against `shared/test-fixtures/`
**When** known queries are run
**Then** same ranked results returned on both Go and Python backends
...
**When** `shared/keywords.json` exists
...
**Then** `shared/keywords.json` is overwritten with new content (ARCH-9)
**And** in-memory keyword map on both backends reloads immediately — no restart required
...
**Given** same `GET /keywords` request sent to Go and Python backends
**When** both respond
**Then** both return identical keyword map from the shared file (parity validation)
