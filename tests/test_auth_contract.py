...
def test_user_model_matches_users_table_contract() -> None:
    ...
    assert {"id", "email", "password_hash", "created_at", "updated_at"}.issubset(User.__table__.columns.keys())


def test_login_schema_matches_previous_contract() -> None:
    request = LoginRequest(email="ces.staff@example.com", password="correct-password")
    response = LoginResponse(token="token", expires_at=int(time.time()))

    assert request.model_dump() == {"email": "ces.staff@example.com", "password": "correct-password"}
