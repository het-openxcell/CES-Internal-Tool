...
from src.services.occurrence.classify import classify_section, classify_type
...
def test_classify_type_returns_matching_type() -> None:
    ...
    assert classify_type("lost circulation during drilling", keywords) == "Lost Circulation"
...
def test_classify_type_word_order_matters() -> None:
    ...
    keywords = {"stuck pipe": "Stuck Pipe", "lost circulation": "Lost Circulation"}
    assert classify_type("pipe stuck in formation", keywords) == "Unclassified"
...
def test_classify_type_first_match_wins() -> None:
    ...
    assert classify_type("stuck pipe", keywords) == "Stuck Pipe"
...
def test_classify_type_empty_keywords_returns_unclassified() -> None:
    assert classify_type("stuck pipe in hole", {}) == "Unclassified"


def test_classify_type_empty_text_returns_unclassified() -> None:
    keywords = {"stuck pipe": "Stuck Pipe"}
    assert classify_type("", keywords) == "Unclassified"


def test_classify_type_multiword_keyword() -> None:
    keywords = {"lost circulation": "Lost Circulation"}
    assert classify_type("experienced lost circulation at 2000m", keywords) == "Lost Circulation"


def test_classify_section_surface_boundary() -> None:
    assert classify_section(0.0) == "Surface"
    assert classify_section(600.0) == "Surface"


def test_classify_section_intermediate_boundary() -> None:
    assert classify_section(600.1) == "Int."
    assert classify_section(2500.0) == "Int."


def test_classify_section_main() -> None:
    assert classify_section(2500.1) == "Main"
    assert classify_section(5000.0) == "Main"
...
def test_classify_section_custom_shoes() -> None:
    ...
    assert classify_section(500.0, surface_shoe=500.0, intermediate_shoe=2000.0) == "Surface"
    assert classify_section(501.0, surface_shoe=500.0, intermediate_shoe=2000.0) == "Int."
    assert classify_section(1500.0, surface_shoe=500.0, intermediate_shoe=2000.0) == "Int."
    assert classify_section(2000.0, surface_shoe=500.0, intermediate_shoe=2000.0) == "Int."
...
def test_classify_section_raises_on_inverted_shoes() -> None:
    with pytest.raises(ValueError, match="surface_shoe"):
        classify_section(1000.0, surface_shoe=2000.0, intermediate_shoe=500.0)


def test_classify_section_raises_when_shoes_equal() -> None:
    with pytest.raises(ValueError, match="surface_shoe"):
        classify_section(500.0, surface_shoe=600.0, intermediate_shoe=600.0)
